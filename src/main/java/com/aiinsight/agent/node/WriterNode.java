...
public class WriterNode implements AgentNode {
    ...
    private String generateWithLlm(AnalysisRun run) {
        ...
        String prompt = """
                ...
                7. 总字数控制在 1200-1800 字。必须包含一个“建议优先级”表，列出：建议、理由、证据、置信度、下一步。
                8. 必须优先使用“结构化结论”和竞品画像；证据索引只用于引用定位，不用于重新分析。
                9. 先根据“用户需求、分析维度、结构化结论、竞品画像”归纳本次最重要的 3-6 个对比维度，再决定报告小节；不要把示例结构当成固定模板。
                10. 报告必须采用单一骨架，按顺序输出且不要新增重复章节：
                    - 一句话结论
                    - 建议优先级
                    - 关键洞察
                    - 竞品能力矩阵
                ...
                28. “竞品能力矩阵”按用户指定维度动态生成列/行，不能写死某个行业、竞品或维度；每格只写判断、关键证据和置信度，不要再追加第二套矩阵。
                29. 文末“结论与建议”只做最终取舍收口：首选借鉴方向、暂不建议投入的方向、最终决策口径。不要重复展开风险清单或行动计划；风险写在“风险与证据缺口”，行动写在“下一步验证计划”。
    ...
    private String decisionSummarySection(AnalysisRun run) {
        ...
        return """
                ## 结论与建议
                - %s
                - 暂不建议投入：证据不足或仍待验证的能力不进入当前确定借鉴范围，先保留为观察项。
                - 最终决策口径：以“建议优先级”和“竞品能力矩阵”为主，风险与验证动作分别参考前文对应章节。
