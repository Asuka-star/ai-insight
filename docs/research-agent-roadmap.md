...
- 抓取失败但搜索摘要可用时保存 `search_result_snippet`，并在 `complianceNote` 中说明。
...
短期可先新增 `SurveyInsight`，支持用户手动上传问卷结果文本、JSON 或 CSV 后结构化解析。
...
- 每个平台认证、API、回调机制不同。
- 对比赛 MVP 来说，先支持用户上传问卷结果更划算。
- 后续可以在 `SurveyProvider` 抽象上扩展。
